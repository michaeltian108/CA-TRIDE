...
class AdvRankLauncher(object):
    ...
    def __call__(self, model: object, loader: object, *, maxiter: int = None):
        ...
        print('[bold green]Original Example:')
        sorig = {key: np.mean([y[key] for y in Sumorig])
                 for key in Sumorig[0].keys()}
        print(utils.rjson(json.dumps(sorig, indent=2)))
        print('[bold red]Adversarial Example:')
        sadv = {key: np.mean([y[key] for y in Sumadv])
                for key in Sumadv[0].keys()}
        print(utils.rjson(json.dumps(sadv, indent=2)))
        print('[bold yello]Difference:')
        ...
        print(utils.rjson(json.dumps(diff, indent=2)))
        print('[yellow]====================================================')
        return (sorig, sadv)
