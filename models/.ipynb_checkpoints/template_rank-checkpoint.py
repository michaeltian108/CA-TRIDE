...
import sys
sys.path.append('/home/tianqiwei/rob_IR/')
...
class MetricBase(thl.LightningModule):
    ...
    def setup(self, stage=None):
        ...
        '''
        train, test = getattr(
           utility, self.dataset).getDataset(self.datasetspec)
    ...
    def training_step(self, batch, batch_idx, optimizer_idx=None):
        ...
        if getattr(self, 'is_advtrain', False):
            ...
            return defenses.pnp_training_step(self, batch, batch_idx, self.trainer.current_epoch, self.trainer.max_epochs)
        elif getattr(self, 'is_advtrain_pnpf', False):
            return defenses.pnp_training_step(
                self, batch, batch_idx, self.trainer.current_epoch, self.trainer.max_epochs, pgditer=1)
    ...
    def validation_epoch_end(self, outputs: list):
        ...
        # log and print
        train_log_filename = 'train_log_'+self.BACKBONE+'_'+self.dataset+ '.txt'#+self.collapse_method+'_'+self.beta
        train_log_filepath = os.path.join('/home/tianqiwei/rob_IR/logs_outputs/',train_log_filename)
        train_log_txt_formatter = "[Epoch] {epoch:03d} [R@1]{r_at1}\n"
        to_write = train_log_txt_formatter.format(
