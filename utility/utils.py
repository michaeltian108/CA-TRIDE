...
def metric_get_rank(dist: th.Tensor, label: th.Tensor,
    ...
    inter_clus_dis = dist[argsort][vallabels[argsort] != label].clone().detach().cpu().data
    inter_clus_dis = inter_clus_dis[inter_clus_dis!=0].numpy().tolist()
    intra_clus_dis = dist[argsort][vallabels[argsort] == label].clone().detach().cpu().data
    intra_clus_dis = intra_clus_dis[intra_clus_dis!=0].numpy().tolist()
    return (rank, inter_clus_dis, intra_clus_dis,) + tuple(rank < k for k in ks)
